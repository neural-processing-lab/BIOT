...
class GwilliamsLoader(torch.utils.data.Dataset):
    def __init__(self, split):
        ...
        if split == "train":
            self.data = Gwilliams2022(
                data_path="/data/engs-pnpl/datasets/gwilliams2022",
                preproc_path="/data/engs-pnpl/datasets/gwilliams2022",
                l_freq=0.5,
                h_freq=100,
                resample_freq=200,
                notch_freq=50,
...
class CamCANUnsupervisedLoader(torch.utils.data.Dataset):
    def __init__(self, split, truncate=0):
        self.meg_files = list(
            glob.glob(f'/data/engs-pnpl/datasets/shafto2014/derivatives/dulhan_megalodon_redux/{split}/meg_*.npy')
        )
        random.shuffle(self.meg_files)
